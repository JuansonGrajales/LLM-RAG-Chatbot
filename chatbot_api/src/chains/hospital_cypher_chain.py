...
hospital_cypher_chain = GraphCypherQAChain.from_llm(
    cypher_llm=ChatOpenAI(model=HOSPITAL_CYPHER_MODEL, temperature=0), # the LLM used to generate Cypher queries
    qa_llm=ChatOpenAI(model=HOSPITAL_QA_MODEL, temperature=0), # the LLM used to generate human-readable responses
    graph=graph, # the Neo4jGraph object that connects to the Neo4j instance
    verbose=True, # whether to print the Cypher query and human-readable response to the console
    qa_prompt=qa_generation_prompt, # the prompt used to generate human-readable responses
    cypher_prompt=cypher_generation_prompt, # the prompt used to generate Cypher queries
    validate_cypher=True, # corrects simple syntax errors in Cypher queries, not a guarantee of correctness
    top_k=100, # the number of query results to return
)
