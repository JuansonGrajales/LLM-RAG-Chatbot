...
def _get_current_hospitals() -> list[str]:
    """Get a list of current hospital names from a Neo4j database."""
    graph = Neo4jGraph(
    ...
    current_hospitals = graph.query("""MATCH (h:Hospital) RETURN h.name AS hospital_name""")
...
def get_most_available_hospital(_: Any) -> dict[str, float]:
    ...
    current_hospitals = _get_current_hospitals()
    wait_times = [_get_current_wait_time_minutes(hospital) for hospital in current_hospitals]
    shortest_wait_time = min(wait_times)
    hospital = current_hospitals[wait_times.index(shortest_wait_time)]
    return {hospital: shortest_wait_time}
